...
class CRExecutionContext {
  ...
  async rawEvaluateJSON(expression) {
    // rebrowser-stealth: inject stealth script before evaluation
    if (process.env['REBROWSER_STEALTH_MODE'] !== '0' && this._contextId > 0) {
      try {
        await this._client.send('Runtime.evaluate', {
          expression: this._getStealthScript(),
  ...
  // rebrowser-stealth: comprehensive stealth script for Playwright
  _getStealthScript() {
    return `
      (() => {
        ...
        // Enhanced user agent with latest Chrome version
